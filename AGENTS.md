...
### ビルドについて
- 実行コマンド `$ node scripts/build.mjs` を実行すると、Firefox と Chrome の両方がビルドされる。
- Firefoxの場合
  - はじめに必要なファイルを /dist/firefox/tmp 配下にコピーをする
  - この時、隠しファイル .DS_Store がコピーされる場合があるため、削除する
  - この tmp フォルダをzipパッケージングし、/dist/firefox フォルダにxpiとして生成
  - 最後に、途中の tmp はフォルダごと削除する
- Chromeの場合
  - はじめに必要なファイルを /dist/chrome/ 配下にコピーをする
  - この時、隠しファイル .DS_Store がコピーされる場合があるため、削除する
